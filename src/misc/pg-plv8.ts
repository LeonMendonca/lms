...
CREATE OR REPLACE FUNCTION create_student_id()
  ...
  const instituteName = NEW.instituteName;
  const departmentName = NEW.department;

  const instituteCount = plv8.execute('SELECT COUNT(*) FROM students_info WHERE "instituteName" = $1', [instituteName])[0].count;
  const instituteCountAsNum = Number(instituteCount) + 1;
 
  const deptCount = plv8.execute(
    'SELECT COUNT(*) FROM students_info WHERE department = $1 AND "instituteName" = $2',
  ...
  const instituteNameAbbr = instituteName.match(/[A-Z]/g)?.join('') ?? '';
  const deptNameAbbr = departmentName.match(/[A-Z]/g)?.join('') ?? '';

  const studentId = instituteNameAbbr+instituteCountPadstart+'/'+deptNameAbbr+deptCountPadstart;

  plv8.execute('UPDATE students_info SET "barCode" = $1 WHERE "studentUuid" = $2', [studentId, NEW.studentUuid]);

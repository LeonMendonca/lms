...
export class ConfigService {
    ...
    try {
      ...
      if (existingInstitute.length > 0) {
        throw new HttpException(
          'Institute With Same ID Exists',
          HttpStatus.BAD_REQUEST,
        );
      }

      // Generate Institute Abbreviation
      const institute_abbr = instituteName
        .split(' ')
      ...
      const queryArgs = insertQuery.values
        .map((_, i) => `$${i + 1}`)
        .join(', ');

      // After inserting into institute_config
      const result = await this.instituteConfigRepository.query(
        `INSERT INTO institute_config (${insertQuery.queryCol}) VALUES (${queryArgs}) RETURNING *`,
        sanitizedValues,
      );
      ...
      await this.instituteConfigRepository.query(
        `UPDATE users_table SET institute_details = $1 WHERE user_uuid = $2`,
        [
          JSON.stringify(created_by_uuid[0].institute_details),
          created_by_uuid[0].user_uuid,
        ],
      );
  ...
  async updateRule(updateLibraryPayload: TLibraryUpdateDTO) {
    try {
      // Generate query for update
      const queryData = updateQueryHelper<TLibraryUpdateDTO>(

...
import {
  ...
  TInstituteDTO,
} from './zod-validation/create-institute-zod';
import {
  TInstituteUpdateDTO,
  updateInstituteSchema,
} from './zod-validation/update-institute-zod';
import {
  createLibraryRuleSchema,
  TLibraryDTO,
} from './zod-validation/create-library_rules-zod';
import {
  TLibraryUpdateDTO,
...
export class ConfigController {
  ...
  async updateRule(@Body() updateLibraryPayload: TLibraryUpdateDTO) {
    try {
      const result = await this.configService.updateRule(updateLibraryPayload);
      return result;
    } catch (error) {
      if (!(error instanceof HttpException)) {
        throw new HttpException(
